...
export interface Database {
  public: {
    ...
    Views: {
      // Safe public projection of profiles (migration 025) — the base table
      // is no longer publicly readable; name/avatar lookups go through this.
      profiles_public: {
        Row: {
          id: string
          full_name: string | null
    ...
    Functions: {
      ...
      // Deletes the calling user's auth row after snapshotting their name
      // onto their reviews (migration 027).
      delete_my_account: {
        Args: Record<string, never>
        Returns: void
      }
      // Anonymous-safe referral-code existence check (migration 025).
