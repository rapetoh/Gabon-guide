-- Migration 021: track which place a coupon was redeemed at.
--
-- For single-place coupons we used to derive "redeemed at place X" by joining
-- coupons.place_id. With platform coupons (migration 020) that column can be
-- NULL, so owner-side activity feeds silently dropped every platform-coupon
-- redemption: the join on places filtered them out.
--
-- Fix: store the place on the redemption row itself. The RPC writes
-- p_place_id when it finalizes a redemption (single-place or platform);
-- existing rows are backfilled from coupons.place_id where possible.

ALTER TABLE public.coupon_redemptions
  ADD COLUMN IF NOT EXISTS place_id uuid REFERENCES public.places(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS coupon_redemptions_place_idx
  ON public.coupon_redemptions (place_id)
  WHERE place_id IS NOT NULL;

-- Backfill: for redemptions of single-place coupons, copy the coupon's place.
-- Platform coupons (coupon.place_id IS NULL) can't be backfilled — those rows
-- stay NULL until they're touched by the RPC again, which won't happen for
-- already-redeemed rows. Acceptable: any pre-021 platform redemptions stay
-- unattributed in owner history.
...
-- Re-emit the RPC with the one-line addition: write place_id on the redemption.
