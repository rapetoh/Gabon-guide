...
### Coupon broadcast + proximity decision (2026-07-18, migration 045)
Founder call, agent owns execution: **(a) SHIPPED — premium coupon broadcast**: places whose tier has the new `coupon_broadcast` feature (premium=on by default, admin-flippable in the tier matrix) notify EVERY non-blocked user when they post a coupon; standard tier keeps favoriters-only; sim-verified (6/6 reach, owner excluded, standard stays at 1 favoriter). Sellable as the premium megaphone. **(b) COMMITTED NEXT — proximity notifications**: geofence-based "you're near {place}, coupon running" pings. Agreed shape: strict opt-in toggle (needs iOS "Always" location), coupon-tied only, max 1/day/user, premium-gated, place list = nearest ~20 coupon-running venues rotated via expo-location geofencing + TaskManager, local notifications (no server round-trip). This is the next major build item — do it in a fresh session, before store screenshots if time allows, and mind Apple review framing (permission copy sells the benefit).

### Proximity alerts SHIPPED (2026-07-18, migration 046 — founder overrode the wait-recommendation, agent executed)
`get_proximity_targets(lat,lon)` RPC returns the ~18 nearest active places with a LIVE coupon whose tier has `proximity_pings` (premium default; both admin matrices show "Alertes de proximité" + "Coupons notifiés à tous"). Client `lib/proximity.ts`: guarded module-scope geofence task (expo-task-manager NEW native module — binaries ≤ #17 no-op), 150m fences, LOCAL notifications, 1/day global + 7-day per-place caps, fences re-aimed on every app foreground. Opt-in toggle in profile Preferences ("Bons plans à proximité") with Always-permission flow + graceful denial alert. Location permission copy updated in app.config (mentions nearby deals — Apple review framing). Verified: RPC sim (expiry + tier dropout), toggle renders on simulator, app boots on old binaries. NOT yet verified: a real geofence entry ping — needs build #18 on a device walking into a fence (or simctl location simulation on a new dev client). Build #18 launched with expo-task-manager.
...
### Content seed (2026-07-19)
58 REAL Libreville venues seeded (web-researched + source-verified: names, zones, GPS in Gabon bounds, +241 phones, FR/EN descriptions) across restaurants/cafés/bars/nightclubs, each with 2 licence-free stock photos (Pexels, theme-matched) in place-photos under seed/<slug>/. All activated through the publish-requires-photo guard. Videos added same day: 1 theme-matched Mixkit clip per venue (360p mp4 ≤15MB, Mixkit license = free commercial use) in place-videos/seed/<slug>/0.mp4, inserted as admin (video tier guard correctly blocked the plain insert). Media is PLACEHOLDER by design — venue-authentic photos/videos come from owners at partner onboarding (never scraped). Seeding used a temporary seed_media edge function, now disabled (410). Venues dataset: scratchpad venues.json (also re-derivable from PLAN). No owners assigned; all tier=free. Founder review fixes (same day): tier spread applied (8 premium incl. Voile Rouge/Sakura/Odika + WhatsApp numbers on paid tiers, 12 standard; Sakura expires J+4 and PAUL J+6 to exercise admin expiry alerts), 2 live coupons on premium places (broadcast fired: 14 notifications), 12 OLD dev-era test places had out-of-Gabon coords (created abroad pre-guard) — all relocated into Libreville. Open-in-Maps flow VERIFIED on simulator end-to-end (place → sheet → Apple Maps with route). Founder guide artifact: tester guide v2 published (claude.ai artifact, per-persona accordions).
