...
- Interim no-push build was ALSO impossible: expo-notifications requests aps-environment just by being installed (auto-linked plugin), and uninstalling it breaks the bundle (Metro resolves the dynamic import statically). Net: **TestFlight stays on build #9 (pre-notification UI) until the founder's one interactive build** — which then ships everything including push. Plugin left in place; builds 13–15 errored, expect the next successful build to be ~#16.
...
3. **Push build (2 min, UNBLOCKS ALL MOBILE SHIPPING)**: run `cd mobile && npx eas-cli build -p ios --profile production` in your terminal, answer **Yes** to "log in to your Apple account" and complete 2FA. (No config edit needed — the plugin is already in place.) EAS regenerates the profile with the Push capability (key 423ZGLXN46 already assigned) and the build proceeds; then `npx eas-cli submit -p ios --latest`. After it's on your iPhone: accept the push prompt and have the agent fire a test notification.
